...
def encode(datum):
    return tf.keras.utils.to_categorical(datum)
...
for train, test in kfold.split(x, decode(y)):
    ...
    for i in range(number_inner_layers):
        model.add(
            tf.keras.layers.Dense(int(number_inner_neurons / 2), input_dim=x.shape[1], activation=inner_activation_fun))
    model.add(tf.keras.layers.Dense(y.shape[1], activation=outer_activation_fun))
